...
def gaussian_nll_loss(input, mean, logvar):
    constant = math.log(2 * math.pi)
    diff = (input - mean).pow(2) / (torch.exp(logvar) + 1e-6)
    loss = 0.5 * (constant + logvar + diff)
...
def train(args, loader, generator, discriminator, vae, g_optim, d_optim, g_ema, device):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vae_regularize', type=float, default=0.01)
    parser.add_argument('--vae_warmup', type=int, default=1000)
    ...
    train(args, loader, generator, discriminator, vae, g_optim, d_optim, g_ema, device)
